from __future__ import annotations

import argparse
import re
import sys
from pathlib import Path

import cv2
import pandas as pd

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))

from nrityalens.analyzer import HAND_FEATURE_COLUMNS, extract_hand_features


IMAGE_EXTENSIONS = {".jpg", ".jpeg", ".png", ".webp", ".bmp"}
DEFAULT_DATASET = ROOT / "data" / "external" / "Bharatanatyam-Mudra-Dataset"
DEFAULT_OUTPUT = ROOT / "data" / "mudra_features.csv"
DEFAULT_LABELS = [
    "Alapadmam",
    "Anjali",
    "Ardhapathaka",
    "Mayura",
    "Mrigasirsha",
    "Mushti",
    "Padmakosha",
    "Pathaka",
    "Shukatundam",
    "Sikharam",
    "Suchi",
    "Tripathaka",
]


def normalize_label(folder_name: str) -> str:
    cleaned = re.sub(r"\(\d+\)$", "", folder_name).strip()
    return cleaned.lower().replace(" ", "_")


def iter_images(dataset_dir: Path, wanted_labels: set[str]):
    for label_dir in sorted(path for path in dataset_dir.iterdir() if path.is_dir()):
        label = normalize_label(label_dir.name)
        if wanted_labels and label not in wanted_labels:
            continue
        for image_path in sorted(label_dir.rglob("*")):
            if image_path.suffix.lower() in IMAGE_EXTENSIONS:
                yield label, image_path


def main() -> None:
    parser = argparse.ArgumentParser(description="Extract hand landmark features from the Bharatanatyam mudra dataset.")
    parser.add_argument("--dataset-dir", type=Path, default=DEFAULT_DATASET)
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument(
        "--labels",
        nargs="*",
        default=DEFAULT_LABELS,
        help="Folder labels to include. Defaults to the app-supported mudras. Use --labels with no values for all labels.",
    )
    parser.add_argument("--limit-per-class", type=int, default=0, help="Optional cap per label for faster experiments.")
    args = parser.parse_args()

    if not args.dataset_dir.exists():
        raise SystemExit(f"Dataset folder not found: {args.dataset_dir}")

    wanted_labels = {normalize_label(label) for label in args.labels}
    rows = []
    skipped = []
    counts: dict[str, int] = {}

    for label, image_path in iter_images(args.dataset_dir, wanted_labels):
        if args.limit_per_class and counts.get(label, 0) >= args.limit_per_class:
            continue

        image_bgr = cv2.imread(str(image_path))
        if image_bgr is None:
            skipped.append((image_path, "could not read image"))
            continue

        image_rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
        features = extract_hand_features(image_rgb)
        if features is None:
            skipped.append((image_path, "no hand detected"))
            continue

        rows.append({"label": label, "image_path": str(image_path.relative_to(ROOT)), **features})
        counts[label] = counts.get(label, 0) + 1

    columns = ["label", "image_path", *HAND_FEATURE_COLUMNS]
    args.output.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(rows, columns=columns).to_csv(args.output, index=False)

    print(f"Wrote {len(rows)} rows to {args.output}")
    for label, count in sorted(counts.items()):
        print(f"- {label}: {count}")
    if skipped:
        print(f"Skipped {len(skipped)} images")
        for path, reason in skipped[:20]:
            print(f"- {path}: {reason}")


if __name__ == "__main__":
    main()
